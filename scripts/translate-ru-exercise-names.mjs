...
const sourcePath = join(root, 'scripts', 'exercise-name-sources', 'ru.json')
const glossaryPath = join(root, 'scripts', 'exercise-name-sources', 'GLOSSARY.ru.md')
...
for (let start = 0; start < pending.length; start += batchSize) {
  ...
  const prompt = `Translate every exercise title below into concise, natural Russian for a gym app.

Rules:
- Return only the structured JSON required by the schema, with every ID exactly once and in input order.
- Return only the Russian title in "name". Do not append or repeat the English title.
- Preserve exercise identity, equipment, stance, grip, direction, side, assisted/weighted status and version qualifiers.
- Prefer the terminology a Russian-speaking gym actually uses (for example: жим, тяга, приседание, сгибание, разгибание, подъём, выпад, отжимание, подтягивание, становая тяга, махи).
- Keep established loanwords where they are the normal spoken term: бёрпи, пилатес, кроссовер, гакк-приседание, EZ-гриф, машина Смита. Do not invent a calque where the loanword is what people say.
- Use sentence case, not Title Case, and no final full stop.
- Keep the English word order — equipment, then movement, then qualifiers — so the list stays scannable beside the English original the app shows in parentheses.
- Use ё where it belongs: подъём, лёжа, бёрпи.
