...
export async function setLang(l, showEn) {
  if (!LANGS[l]) l = 'en'
  const show = showEn !== false
  if (l === getLang() && getVersion() > 0 && show === lastShowEn) return
  ...
  _setLangState(l, derivePack(l, dict), derivePack(l, instr), derivePack(l, exerciseNames), show)
  notify()
}
